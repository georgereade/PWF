...
def process_time_per_contact(trackedfrom, trackedto):
    ...
    for contact in staff_contacts:
        ...
        for record in time_data:
            ...
            if category_name in ['On Hold','Current Timed Projects']:result.append({
                'Staff': contact_name,
...
def main():
    ...
    result = process_time_per_contact(trackedfrom, trackedto)

    # Convert the result into a DataFrame and save it to an Excel file
    df = pd.DataFrame(result)
    excel_file = f'output/staff_time_per_project{trackedfrom}-{trackedto}.xlsx'
    ...
    print(f"Time data saved to {excel_file}")
